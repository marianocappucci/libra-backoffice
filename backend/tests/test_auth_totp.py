...
from .conftest import login, PASSWORD, TOKEN, USUARIO, _TransporteDeInstancias, construir_settings
...
def test_sin_secreto_un_codigo_de_mas_no_molesta(cliente):
    resp = login(cliente, {"username": USUARIO, "password": PASSWORD, "codigo": "000000"})
...
class TestConSegundoFactor:
    ...
    def test_codigo_incorrecto_es_el_mismo_401_que_clave_incorrecta(self, con_totp):
        ...
        a = login(con_totp, {"username": USUARIO, "password": PASSWORD, "codigo": "123456"})
        b = login(con_totp, {"username": USUARIO, "password": "mal", "codigo": CODIGO_AHORA})
        assert a.status_code == b.status_code == 401
        assert a.json() == b.json()

    def test_con_codigo_correcto_entra(self, con_totp):
        resp = login(con_totp, {"username": USUARIO, "password": PASSWORD, "codigo": CODIGO_AHORA})
        assert resp.status_code == 200
        assert con_totp.get("/api/me").status_code == 200

    def test_el_mismo_codigo_no_entra_dos_veces(self, con_totp):
        assert login(con_totp, {"username": USUARIO, "password": PASSWORD, "codigo": CODIGO_AHORA}).status_code == 200
        con_totp.post("/api/logout")
        assert login(con_totp, {"username": USUARIO, "password": PASSWORD, "codigo": CODIGO_AHORA}).status_code == 401

    def test_los_intentos_fallidos_con_codigo_tambien_cuentan_para_el_bloqueo(self, con_totp):
        for _ in range(5):
            login(con_totp, {"username": USUARIO, "password": PASSWORD, "codigo": "000000"})
        resp = login(con_totp, {"username": USUARIO, "password": PASSWORD, "codigo": CODIGO_AHORA})
...
def test_el_bloqueo_sobrevive_a_un_reinicio_del_backoffice(monkeypatch, tmp_path, instancias_falsas, inventario):
    ...
    with _cliente(tmp_path, instancias_falsas, inventario) as c:
        for _ in range(5):
            login(c, {"username": USUARIO, "password": "mal"})
        assert login(c, {"username": USUARIO, "password": PASSWORD}).status_code == 429
    # "Reinicio": otra app, otro AdminAuth, mismo archivo.
    with _cliente(tmp_path, instancias_falsas, inventario) as c2:
        assert login(c2, {"username": USUARIO, "password": PASSWORD}).status_code == 429
